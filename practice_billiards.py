...
벽을 기준으로 목표 공을 대칭 이동시키기
왼쪽 벽 (-x,y)
오른쪽 벽 (2*가로길이-x,y)
위쪽 벽 (x,2*세로길이-y)
아래 벽 (x, -y)

거리**2 = (x1-x2)**2 + (y1-y2)**2
여기서 벽 부딪히기 전 맞추면 예외처리
'''
def solution(m, n, startX, startY, balls):
    ...
    for targetX, targetY in balls:
        distance = []
        # 왼쪽 벽 대칭
        # 단, 두 공의 Y축이 같은데 목표 공이 내 공보다 왼쪽에 있다면 먼저 부딪히므로 제외
        if not (startY == targetY and startX > targetX):
            distance.append((startX+targetX)**2 + (startY-targetY)**2)
        # 오른쪽 벽 대칭 -> 여기도 똑같이 Y축 같은데 목표공이 오른쪽있으면 부딪히니까 제외
        if not (startY == targetY and startX < targetX):
            distance.append((startX-(2*m - targetX))**2 + (startY-targetY)**2)
        
        # 위쪽 벽 대칭 -> X축 같은데 목표가 위에있는거 제외
        if not (startX == targetX and startY < targetY):
            distance.append((startX-targetX)**2 + (startY-(2*n-targetY))**2)
        # 아래 벽 대칭 -> X 같고 목표 아래 제외
        if not (startX == targetX and startY > targetY):
            distance.append((startX-targetX)**2 + (startY+targetY)**2)
