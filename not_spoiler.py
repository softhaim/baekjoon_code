...
def solution(message, spoiler_ranges):
    ...
    for start, end in spoiler_ranges:
        # 앞쪽으로 검사하면서 단어의 시작(공백) 찾기
        while message[start] != " " and start > 0 and message[start - 1] != " ":
            start -= 1
        # 뒤쪽으로 검사하면서 단어의 끝(공백) 찾기
        while message[end] != " " and end < len(message)-1 and message[end+1] != " ":
            end += 1
        # 스포 방지 구간의 word를 추출해 저장
        for word in message[start:end+1].strip().split():
            spoiler_word.append(word)
        # 단어 스포일러 방지 처리된 부분은 공백으로 제거해 줌
        message = message[:start] + " "*(end-start+1) + message[end+1:]
    # 스포일러 방지 처리된 message 부분을 카운터로 개수 셈
    word_counter = Counter(message.split())
    # 스포일러 방지 word 부분을 돌면서 해당 word가 기존에 안나온 거면 + 1 하고 스포일러 제거하면서 counter 에 + 1
