...
def solution(schedules, timelogs, startday):
    answer = 0
    for idx, hope_time in enumerate(schedules):
        fix_hope_time = hope_time + 10
        if fix_hope_time%100 >= 60: # 60분을 넘어가면 올림 이루어져야 함.
            fix_hope_time = ((fix_hope_time//100)+1)*100 + (fix_hope_time%100)-60
        cnt = 0 # 해당 주에 몇번 제대로 출근 했는지 count

        for val in timelogs[idx]:
            startday = startday % 7
            if startday == 6 or startday == 0: # 토, 일 인경우
                startday += 1
                continue
            if fix_hope_time >= val: cnt += 1
            startday += 1
