...
5개로 만들 수 있는 조합을 해보면서 이 조합이 입력 정수의 시스템 응답 수와 일치하는 조합이라면 cnt += 1
'''
...
def solution(n, q, ans):
    ...
    for comb in combinations(range(1, n+1), 5):
        exact_system_answer_count = 0
        for idx, input_arr in enumerate(q):
            match_count = len(set(comb) & set(input_arr))
            if match_count == ans[idx]:
                exact_system_answer_count += 1
        if exact_system_answer_count == len(q):
            answer += 1
    return answer